...
**비밀번호 재설정 흐름**
1. 일반 계정은 로그인 화면의 **비밀번호 찾기**에서 가입 이메일을 입력하면 Firebase 기본 재설정 메일이 발송됩니다.
2. 관리자 계정은 로그인 ID가 `실제메일앞쪽_admin@mysc.co.kr` 형식입니다.
3. 관리자 계정이 비밀번호를 잊은 경우에도 동일하게 **비밀번호 찾기**에서 관리자 로그인 ID를 입력합니다.
4. 시스템은 `_admin`을 제거한 실제 메일 주소로 재설정 메일을 발송합니다.
   - 예: `mwbyun1220_admin@mysc.co.kr` 입력 시 `mwbyun1220@mysc.co.kr`로 발송
5. 어드민이 로그인 가능한 상태라면 **사용자 관리**에서 다른 관리자 계정의 재설정 메일을 대신 발송할 수도 있습니다.

QA 체크 포인트
- 가입 직후 **가입 승인 대기 목록**에 나타나는지 확인
- 어드민 승인 후 역할이 올바르게 적용되는지 확인
- 승인 전에는 역할 메뉴가 제한되는지 확인
- 관리자 계정 비밀번호 찾기 시 `_admin`이 제거된 실제 메일 주소로 안내 문구가 표시되는지 확인
...
5. 관리자 계정(`_admin@mysc.co.kr`)은 **재설정** 버튼으로 실제 복구 메일에 비밀번호 재설정 링크를 발송할 수 있음
